"""FP8 flash-attention via a Triton kernel on H100 (Hopper / sm_90a).

This is the AttentionOp wrapper for the fused FP8 flash-attention kernel in
``kernels/triton_kernels/fp8_flash_attn_hopper.py``.  The kernel uses
Hopper's native FP8 wgmma instructions (``wgmma.mma_async.*.f32.e4m3.e4m3``)
through Triton's ``tl.dot`` with FP8 operands; the running softmax
statistics and accumulator stay in FP32.

The fused kernel is the right shape for FP8 attention on long sequences
(the Cosmos DiT runs S up to ~109k tokens, head_dim 128).  The unfused
``fp8_scaled_mm`` path materializes the S^2 scores matrix and is
bandwidth-bound at that scale; the fused tile-by-tile loop streams K/V
through registers and never touches HBM for the scores.

This is the Hopper sibling of ``fp8_triton.py`` (CDNA3 / gfx942).  The
underlying algorithm is identical (FA-2 online softmax); the divergences
are FP8 dtype (e4m3fn vs e4m3fnuz), FP8 range (448 vs 240), and an
autotune grid biased to Hopper's wider SMs + 228 KiB SMEM budget.
FlashAttention-3 IS available on Hopper and is a legitimate alternative,
but the Triton path gives us per-shape autotune control and a single
source of truth across the two backends.  See ADR-0002.

See ADR-0002 and ``kernels/triton_kernels/fp8_flash_attn_hopper.py`` for
the algorithm.
"""
...
class FP8HopperTritonAttention:
    """FP8 fused flash-attention via the kernels/ Triton kernel on Hopper."""

    name = "fp8-hopper-triton-flash"

    def __init__(self) -> None:
        self._fn: Any | None = None
        self._import_error: str | None = None
        try:
            # Kernels live outside src/mirage/; importing via package path so
            ...
            # mirage proper.  See kernels/README.md.
            import sys
            ...
            from triton_kernels.fp8_flash_attn_hopper import fp8_flash_attention
    ...
    def __call__(
        ...
        *,
        ...
        if self._fn is None:
            raise RuntimeError(
                "FP8 Hopper Triton attention is not available "
...
__all__ = ["FP8HopperTritonAttention"]
