"""Custom AMX INT8 flash-attention — the Mirage-owned CPU kernel.

Sibling of :class:`mirage.attention.amx_flash.AMXFlashAttention` (BF16) and
:class:`mirage.attention.fp8_triton.FP8TritonAttention` (gfx942).  Same
...
class AMXInt8FlashAttention:
    """Flash-attention on Intel AMX_INT8.  Mirage-owned C++ kernel.
    ...
    by :mod:`mirage.runtime.quantize` for static weight quant.
    """
    ...
    def __call__(
        ...
        *,
        ...
        # Mirage convention: (B, H, S, D).  The C++ kernel takes the same
