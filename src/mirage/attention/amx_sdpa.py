...
Ordering in :mod:`mirage.attention.registry`:

* :class:`mirage.attention.amx_flash.AMXFlashAttention` — custom C++/AMX
...
* :class:`mirage.attention.ipex_flash.IPEXFlashAttention` — Intel-Extension-
...
* :class:`AMXSDPAAttention` — the fallback.  Always available; correct on
  ...
  integration.
* :class:`mirage.attention.naive.NaiveAttention` — the FP32 naive floor.
...
# numeric dtypes Mirage cares about.  We restrict to the ones oneDNN will
# accelerate on AMX (BF16, FP16, INT8) plus FP32 as a correctness floor.
_SUPPORTED_DTYPES = frozenset(
    {DType.FP32, DType.FP16, DType.BF16, DType.INT8}
)

# All Mirage attention kinds are expressible through SDPA's mask + is_causal
...
class AMXSDPAAttention:
    ...
    def __call__(
        ...
        *,
        ...
        # Mirage convention is (B, H, S, D) — same as SDPA's expected layout
