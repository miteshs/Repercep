...
Where :class:`mirage.runtime.engine.WorldModelEngine` turns one request into one
...
``docs/adr/0008-interactive-world-model-seam.md`` and
``docs/STRATEGIC_ASSESSMENT.md``).

The latent world model (V-JEPA 2-AC, :mod:`mirage.models.vjepa2_ac`) is the lead
...
class InteractiveWorldModel(Protocol):
    ...
    Implementations advance a latent :class:`~mirage.runtime.types.WorldState`
    one action at a time. They reuse the same ``Backend`` seam (ADR-0003) and
    config conventions as :class:`~mirage.runtime.engine.WorldModelEngine`; the
