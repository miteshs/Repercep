...
This backend is the parallel of :class:`mirage.backend.rocm.ROCmBackend`.  AMD
...
class CUDABackend:
    ...
    def capabilities(self) -> BackendCapabilities:
        ...
        # ``mirage info`` and tests as a declarative summary.
        ...
        # selection promotes it when MIRAGE_FP8_ATTENTION=te is set.
        ops: list[str] = []
        if flash.available:
            ops.append(flash.name)
        ops.append("naive-sdpa")
        # FP8 ops are advertised when their kernels are importable; the
        # registry will only route to them under the MIRAGE_FP8_ATTENTION
        # env var.
        try:
