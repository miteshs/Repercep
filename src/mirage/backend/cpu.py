...
Mirage's third compute backend, parallel to ROCm and CUDA.  The motivation
...
* **Generic AVX-512** — pre-AMX Xeon (Ice Lake, Cascade Lake).  Mirage will
...
generation that gets FP8 is not yet public).  ``MIRAGE_AMX_ATTENTION``
mirrors ``MIRAGE_FP8_ATTENTION`` on the GPU side: with ``=1``, the
