"""``python -m mirage.bench`` — run a benchmark from the command line.

Examples:
    # quick sanity benchmark, no model weights needed
    python -m mirage.bench --engine stub

    # the real Cosmos-Predict-7B baseline on the MI300X
    python -m mirage.bench --engine cosmos --frames 121 --steps 36 --warmup 1 --iters 3
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="mirage.bench", description="Benchmark a Mirage world-model engine."
    )
    ...
    from mirage.bench.harness import benchmark_engine
