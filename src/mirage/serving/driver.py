...
— two args. The real :class:`mirage.runtime.scheduler.Scheduler` exposes
...
Fields mirror :class:`mirage.runtime.types.FrameChunk` so a v1→v2 migration
...
class _SchedulerAdapter:
    ...
    ``submit(id, priority)`` (see ``crates/mirage-router/src/lib.rs::529``).
...
def encode_frame_line(
    *,
    ...
    mirrors :class:`mirage.runtime.types.FrameChunk` so v1 and v2 consumers
...
class EngineDriver:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="mirage-engine-driver", daemon=True
        )
        self._thread.start()
