...
it doesn't control, without forking it. Mirage's code: `src/mirage/attention/
diffusers_backend.py`.

## 4.1 The problem: the model bypasses Mirage's own registry (F19/F40)

Mirage has an attention abstraction (`attention/registry.py`, `select_attention_op`).
But **the Cosmos model never calls it** — `dispatch_attention_fn` consults
diffusers' *own* `_AttentionBackendRegistry`. So Mirage can't intercept attention
...
```python
new_member = str.__new__(AttentionBackendName, "mirage_fp8")   # bypass Enum's guard
new_member._name_ = "MIRAGE_FP8"; new_member._value_ = "mirage_fp8"
AttentionBackendName._member_map_["MIRAGE_FP8"] = new_member     # patch the maps
AttentionBackendName._value2member_map_["mirage_fp8"] = new_member  # the lookup reads
```

`register_mirage_fp8_backend` (318) then drops `_mirage_fp8_attention` into
...
`cosmos.py:168`) — sets the active backend iff `MIRAGE_FP8_ATTENTION` is set. This
is what makes one env var govern *both* Mirage's own registry and the diffusers
...
`_mirage_fp8_attention` receives diffusers' `(B, S, H, D)` tensors and decides,
per call, whether the FP8 kernel earns its keep. In order:

1. **`MIRAGE_FP8_ATTENTION=fa`** (193) → route to FlashAttention-2/3 via
...
CosmosAttnProcessor2_0  (builds Q,K,V; QK-norm; RoPE; GQA)        Part 2
   └─ dispatch_attention_fn                                       diffusers dispatcher
        └─ active backend:
             ...
             └─ "mirage_fp8"   → _mirage_fp8_attention (routing)
...
asserts the `"mirage_fp8"` member resolves, registration is idempotent, the env
