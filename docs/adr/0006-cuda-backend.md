...
- **Resolves:** the "Revisit if" clause of ADR-0001 — by promoting NVIDIA
  from fast-follow to *parallel* track now, ahead of the design-partner
...
(a) Defer the NVIDIA port until a design partner forces it (ADR-0001's
    original "Revisit if" trigger — "inference-cloud or AV-simulation
    design partner lands that is H100/H200-bound").
...
- **AMD remains the lead.** The Cosmos / Wan benchmark headlines, the
  `BUILD_LOG.md` chronology, the OSS writeup at
  `docs/COSMOS_ON_MI300X.md`, the strategic framing in `HANDOFF.md`
  — none of these change. The H100 port adds a sibling, not a
...
- **An NVIDIA-specific design partner lands that pulls scope** (e.g.
