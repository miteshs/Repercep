...
Land a third compute backend, `mirage.backend.cpu.CPUBackend`, that satisfies
...
both the AMD MI300X and the NVIDIA H100 RunPod templates Mirage develops
against — every developer who runs Mirage on a GPU also has Sapphire Rapids
sitting underneath.
...
| Op | Module | Role |
|---|---|---|
| `AMXFlashAttention` | `mirage/attention/amx_flash.py` | Custom AMX BF16 flash kernel.  Sibling of the gfx942 + Hopper Triton kernels.  C++ source in `kernels/cpu/amx_attn/`.  Wins at long S because it does not materialise QK^T. |
| `IPEXFlashAttention` | `mirage/attention/ipex_flash.py` | Wrapper over Intel-Extension-for-PyTorch's fused attention.  Available only when IPEX is installed.  Broader shape coverage than the AMX flash kernel. |
| `AMXSDPAAttention` | `mirage/attention/amx_sdpa.py` | `torch.nn.functional.scaled_dot_product_attention` on CPU.  Always available; on AMX-class hardware oneDNN's matmul dispatch picks AMX tiles automatically.  The floor. |

Routing is gated on `MIRAGE_AMX_ATTENTION` (parallels `MIRAGE_FP8_ATTENTION`):
...
2. **CI without a GPU.**  Mirage's pytest suite includes 12 vendor-conditional
...
* **A custom AMX INT8 attention kernel.**  AMX_INT8 (TDPBSSD) is on the
  roadmap as `mirage/attention/amx_int8_flash.py` — sibling of the BF16
...
* `mirage info` now reports a CPU device on every host, with feature
...
* **Use oneDNN directly via its C API.**  Rejected as overkill for the
  ...
  layer between Mirage's shape types and the AMX tile registers without
