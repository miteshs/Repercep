...
- **Status:** Accepted
- **Date:** 2026-05-23
- **Resolves:** the open fork from `~/Mirage_Cowork_Handoff.md` ("Language
  stack" section, "THE ONE FORK TO STRESS-TEST")
- **Supersedes:** the deferred posture in
  [ADR-0004](0004-polyglot-build-tooling.md) (`Rust-core fork deferred`)

## Context

The handoff doc spelled out a binary fork:

> If all of the first 3 customers are datacenter, it's defensible to ship a
> Python-core (vLLM-style) datacenter runtime first and budget the Rust
> hot-path rewrite into Series A — ~2–3mo faster to v0.1, max Claude Code
> leverage. If even one early design partner is a robotics OEM (edge /
> closed-loop / sub-100ms), build Rust-core from day one.

The Python core works (Cosmos-Predict-7B on MI300X at 2.47× the H100
reference, 154 s on the 121 f / 36-step config). No design partner has signed
yet. By the handoff's own logic, Python-core is the *defensible* default.

We are choosing Rust-core anyway, ahead of design-partner mix landing.
...
2. **The robotics-OEM scenario is asymmetric.** If a robotics OEM lands as
   design partner #1 or #2, the handoff projects a 6–9 month cold-start
   ...
   it buys. Even if no robotics OEM ever lands, the cache/scheduler/router
...
4. **Handoff §"Highest-leverage move" already endorses interface discipline
...
5. **Aggressive use of Claude Code agents in parallel makes the cost
   atypical.** The handoff's "1.2–1.5× on novel CUDA/MLIR" figure assumes
...
- **Phase 2 ships >1 month late** and the attribution traces to Rust work:
  ...
  per crate.
- **Series A operational scars suggest a different decomposition** of
