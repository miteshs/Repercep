...
80GB HBM3 VF through Mirage's `WanEngine` + the diffusers `WanPipeline`.
...
|---|---|---|---|
| Stack | Wan2.2 repo + FA-3 + offload + FP8 | `diffusers` 0.37.1 + Mirage WanEngine + VAE tiling | `diffusers` 0.37.1 + Mirage WanEngine |
...
dispatcher (cuDNN flash internally; FA-3 is built and the Mirage
bridge is active via `MIRAGE_FP8_ATTENTION=fa`, but appears not to
...
- **Mirage's BF16, no-offload, both-experts-resident path on H100
  ...
  fades.
- **Mirage with the Wan team's offload + FP8 stack on H100:** open.
...
datapoint: Mirage's both-experts-resident, full-precision-VAE,
...
Two runs, both with `MIRAGE_FP8_ATTENTION=fa`,
...
1. **Mirage's diffusers-path Wan on H100 does NOT fit within the 80 GB
   ...
   Mirage's path needs *either* offload (their lever) or VAE tiling
...
3. **Headroom for a Wan-shaped adaptive cache is large.** With the
   ...
   are an upper bound; single-H100 Mirage with a Wan-shaped native
...
- **NVIDIA-canonical FP8 (TransformerEngine) not exercised here.** TE
  installation completed in Session 16; the TE-via-Mirage path
...
- **Wan-native adaptive cache not exercised.** `WanConfig`'s
  ...
  Cosmos-shape cache in `mirage.runtime.denoise.denoise_cosmos_video`
...
# MIRAGE_FP8_ATTENTION env var below. Even though FA-3 doesn't appear
...
HF_HOME=/workspace/hf-cache MIRAGE_FP8_ATTENTION=fa \
...
inside `AutoencoderKLWan.forward`. `MIRAGE_FP8_ATTENTION=fa` registers
the Mirage attention bridge with the diffusers dispatcher; today this
