...
//!   (which is **not** a Cargo dep on `mirage-scheduler` — the trait is
...
///
/// The real impl lives in `mirage-scheduler`; in this crate we only define
/// the trait and use it generically. There is **no** Cargo dep on
/// `mirage-scheduler` — the trait is the integration seam, and the Python
...
/// pulling in the real `mirage-scheduler` crate. The Python entry point
...
/// `mirage_scheduler.Scheduler` (sister Agent B's crate) without this crate
...
#[pyclass(name = "Router", module = "mirage_router._native")]
pub struct PyRouter {
...
#[pyclass(name = "FrameStream", module = "mirage_router._native")]
...
/// PyO3 module registration: `mirage_router._native`.
#[pymodule]
...
mod tests {
    ...
    /// impl in every Rust unit test — no dep on `mirage-scheduler` needed.
    #[derive(Default)]
